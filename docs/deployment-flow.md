...
8. `systemctl restart ids-platform-backend` restarts the service.
...
The current backend workflow still defaults to `/opt/ids-platform-backend/app.jar`. The recommended infra target is the release-based layout above because it supports rollback and future multi-service consistency.
